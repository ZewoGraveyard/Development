...
class QuarkTests : XCTestCase {
    ...
    func testQuarkErrorDescription() throws {
        XCTAssertEqual(String(QuarkError.invalidConfiguration(description: "foo")), "foo")
        XCTAssertEqual(String(QuarkError.invalidArgument(description: "foo")), "foo")
    }
    ...
    func testLoadCommandLineArguments() throws {
        ...
        let parsed = try load(commandLineArguments: arguments)
        XCTAssertEqual(parsed, ["server": ["log": true, "host": "127.0.0.1", "port": 8080]])

        arguments = ["foo"]
        XCTAssertThrowsError(try load(commandLineArguments: arguments))
    ...
    func testLoadEnvironmentVariables() throws {
        ...
        let parsed = try load(environmentVariables: variables)
    ...
    func testSpawnErorDescription() throws {
        XCTAssertEqual(String(SpawnError.exitStatus(0, [])), "exit(0): []")
    }
