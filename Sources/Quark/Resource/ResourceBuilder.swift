public final class ResourceBuilder : RouterBuilder {
    ...
    init(path: String, viewsPath: String) {
...
extension ResourceBuilder {
    public func get<
        ...
        let mustacheSerializer = MustacheSerializer(templatePath: "Views/\(viewsPath)/\(view)")
...
extension ResourceBuilder {
    public func list<
        ...
        let mustacheSerializer = MustacheSerializer(templatePath: "Views/\(viewsPath)/list.html")
        let templateMiddleware = TemplateEngineMiddleware(serializer: mustacheSerializer)
        let responder = BasicResponder { request in
            let actionOutput = try action()
            do {
