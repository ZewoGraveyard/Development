...
public final class Client : Responder {
    ...
    public init(uri: URI, configuration: Map = nil) throws {
        self.secure = try isSecure(uri: uri)
    ...
    public convenience init(uri: String, configuration: Map = nil) throws {
...
extension Client {
    ...
    private static func getCachedClient(uri: URI) throws -> Client {
        let (host, port) = try getHostPort(uri: uri)
...
fileprivate func isSecure(uri: URI) throws -> Bool {
    let scheme = uri.scheme ?? "http"
...
fileprivate func getHostPort(uri: URI) throws -> (String, Int) {
    let scheme = uri.scheme ?? "http"
    ...
    switch scheme {
    case "http": port = uri.port ?? 80
    case "https": port = uri.port ?? 443
    default: throw HTTPClientError.invalidURIScheme
    }
